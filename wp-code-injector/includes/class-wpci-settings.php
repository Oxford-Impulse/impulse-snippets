...
class Wpci_Settings {
	...
	public function render_page() {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Settings', 'wp-code-injector' ); ?></h1>

			<?php $this->maybe_render_status_notice(); ?>

			<?php if ( $disable_all ) : ?>
				<div class="notice notice-warning inline">
					<p><?php esc_html_e( 'All snippets are currently paused site-wide.', 'wp-code-injector' ); ?></p>
				</div>
			<?php endif; ?>
				...
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					...
					<p>
						<label>
							...
							<strong><?php esc_html_e( 'Pause all snippets (emergency kill switch)', 'wp-code-injector' ); ?></strong>
						</label><br>
						<span class="description"><?php esc_html_e( "Stops every snippet from outputting anywhere on the site, without changing any individual snippet's own on/off setting. Use this if a snippet breaks something and you need to shut everything off immediately.", 'wp-code-injector' ); ?></span>
					</p>
					...
					<p>
						<label>
							...
							<strong><?php esc_html_e( 'Delete all snippets and settings when this plugin is deleted', 'wp-code-injector' ); ?></strong>
						</label><br>
						<span class="description"><?php esc_html_e( 'Off by default, so deleting the plugin by accident never loses your data. Turn this on only if you want a completely clean removal.', 'wp-code-injector' ); ?></span>
					</p>

					<p><button type="submit" class="button button-primary"><?php esc_html_e( 'Save Settings', 'wp-code-injector' ); ?></button></p>
				</form>
			</div>
		</div>
		<?php
	...
	private function maybe_render_status_notice() {
		if ( isset( $_GET['wpci_saved'] ) ) {
			printf( '<div class="notice notice-success is-dismissible"><p>%s</p></div>', esc_html__( 'Settings saved.', 'wp-code-injector' ) );
		}
	}
