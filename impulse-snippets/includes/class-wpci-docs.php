	...
	public function render_page() {
		...
		<div class="wrap wpci-docs">
			...
			<div class="postbox wpci-integration-card" style="max-width:800px;">
				...
				<p><?php esc_html_e( 'Form lead tracking: if Contact Form 7, WPForms, or Elementor Pro is installed, the Conversion tracking panel gains a form-tracking section. Pick a form (Elementor forms are matched by the Form Name set in the widget) and a lead is counted the moment a visitor successfully submits it — a free GA4 "generate_lead" event always, plus a Google Ads conversion if you add a conversion label. No thank-you page is needed and failed submissions never count. Optionally enable the hashed-email checkbox: the email the visitor typed is scrambled in their own browser before being sent (the readable address never goes to Google), which lets Google match leads to ad clicks even for logged-out visitors.', 'impulse-snippets' ); ?></p>
				<p><?php esc_html_e( '"Check my setup" (button at the top of the Integrations page): opens your site with an admin-only panel that verifies each connected integration actually printed its code on the page — only you see it, visitors never do. It can also send a test conversion, but read its warning first: Google counts test conversions in your real campaign statistics.', 'impulse-snippets' ); ?></p>
			</div>
