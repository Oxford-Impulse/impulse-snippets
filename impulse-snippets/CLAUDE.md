...
- WooCommerce purchase tracking (1.17.0): the Google Ads card gains a purchase section when WooCommerce is active; orders are reported on the thank-you page with real total/currency/order number via the `woocommerce_thankyou` hook. Optional enhanced conversions send a SHA-256 hashed email (never raw); page-based conversion actions can do the same for logged-in visitors. Form-plugin capture for logged-out leads is Phase 2 (parked).
...
| `class-wpci-ads-dynamic.php` | Dynamic Google Ads output: WooCommerce purchase conversions on `woocommerce_thankyou` (the `google_ads_purchase` snippet's conditions match nothing — the hook is its targeting) + hashed `user_data` on `wp_head` priority 0 (before Wpci_Output's 1) for enhanced-flagged conversions (`_wpci_ads_enhanced` meta) |
